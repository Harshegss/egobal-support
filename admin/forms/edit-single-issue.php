  ...
  <div class="offcanvas-body">
    <form action="app/single_issue_controller.php" method="POST" enctype="multipart/form-data">
      ...
      <div class="form-feild mt-3">
        ...
        <textarea class="tinymce-editor" name="content"><?= $get['content'] ?></textarea>
      </div>
      <div class="form-feild mt-3">
        <?php foreach (json_decode($get['sec_heading']) as $key => $value) : ?>
          <div class="row bg-light py-3 rounded mb-3 appendedItem2 position-relative">
            ...
            <button class="btn btn-danger btn-sm removeSection position-absolute" style="top:10px; right:10px">Remove</button>
              <input type="hidden" name="sec_image_text[]" value="<?=json_decode($get['sec_image'])[$key]?>">
      ...
      <div class="form-feild mt-3">
        ...
        <input type="text" class="form-control mt-2" name="meta_title" value="<?= $get['meta_title'] ?>">
      </div>
      <div class="form-feild mt-3">
        <label class="text-white">Meta Description</label>
        <textarea class="form-control mt-2" rows="2" name="meta_description"><?= $get['meta_description'] ?></textarea>
      </div>
      <div class="form-feild mt-3">
        <label class="text-white">Meta Keywords</label>
        <input type="text" class="form-control mt-2" name="meta_keyword" value="<?= $get['meta_keywords'] ?>" placeholder="Hello, Hii, Bye, Byee">
      </div>
